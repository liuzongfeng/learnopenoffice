               ...
               <!-- <p style="color: red;">因为是测试所以一种文件只允许上传一个</p> -->
                <div style="padding: 10px">
                    <form enctype="multipart/form-data" id="fileUpload">
                        ...
                        <input type="hidden" name="comNum" id="comNum"/><br/>
...
<script>
    function deleteFile(fileName,comNum) {
        $.ajax({
            url: '${baseUrl}deleteFile?comNum='+comNum+'&fileName=' + encodeURIComponent(fileName),
    ...
    $(function () {
		...
		var comNum = decodeURI(GetQueryString("comNum"));
    	
    	
    	
        $('#table').bootstrapTable({
            url: 'listFiles?comNum='+comNum,
            columns: [{
                field: 'fileName',
                title: '文件名'
            }, {
            ...
            $(data).each(function (index, item) {
                item.action = "<a class='btn btn-default' target='_blank' href='${baseUrl}onlinePreview?url="
                    + encodeURIComponent('${baseUrl}' + item.fileName ) +"'>查看扫描件</a>" +
                    "<a class='btn btn-default' target='_blank' href='javascript:void(0);' onclick='deleteFile(\""+item.fileName+"\",\""+comNum+"\")'>删除</a>";
            });
            return data;
